...
Data_post_pre_tt <- read_rds("data/genere/data_prett.rds") %>% 
  ...
  group_by(id_patient) %>%
  mutate(naEPW = sum(is.na(SYM_fatigueMatinale_echelle)|is.na(Vis_init_SYM_fatigueMatinale_echelle))) %>% # utile car permet de scale sur plus de rendez-vous
  filter(naEPW == 0) %>% 
  select(-naEPW) %>% 
  ungroup()  
...
saveRDS(vec_cut, file = paste0("data/genere/obs_cut_morningT",visite_seuil,".rds"))

saveRDS(data.frame(id_patient = df_obs_categ$id_patient,PPC_observanceMoy_finale = df_obs_categ$PPC_observanceMoy_finale),
        file = paste0("data/genere/obs_morningT_quantit",visite_seuil,".rds"))
...
df_post_imput <- impute_si_changement(df_pre_impute,"data/genere/data_impute_morningT.rds",reimputation = FALSE)
...
saveRDS(df_pre_impute, file = paste0("data/genere/data_pre_impute_morningT_nb_vis_",visite_seuil,".rds"))
saveRDS(df_post_imput, file = paste0("data/genere/data_prett_morningT_nb_vis_",visite_seuil,".rds"))
