...
name_outcome <- paste0(quote(SYM_fatigueMatinale_echelle))
visite_seuil <- 2
# data
df_modele_pds <- read_rds("data/genere/data_prett_morningT_nb_vis_2.rds") 
...
remove_var_model <- c(
  "SYM_somnolenceDiurne","Vis_init_SYM_fatigueMatinale",
  "Vis_init_PSG_saturationMoyenne","Vis_init_PSG_delai_explo"
  ,"PPC_pressionMoyenne","INS_PPC_effet_ind","PPC_id_typeMasque",
  "SYM_fatigueMatinale","Vis_init_PSG_indexMicroEveil","SYM_troubleLibido","SYM_transpiNocturne"
)


Var_model_outcome <- c(select_var$var_select$model_outcome[select_var$var_select$model_outcome %notin% remove_var_model],"Vis_init_SYM_fatigueMatinale_echelle")
...
donnee_boot <- df_modele_pds %>%
  mutate_at(
    typages_function(df_modele_pds, 10)$colonne_type$facteur,
    factor
  )
...
if ("OK" == "pas OK") {
  ...
  saveRDS(boot_strap_full_process, file = paste0("data/genere/bott_strap_full_process_morningT.rds"))
}



boot_strap_full_process <- readRDS("data/genere/bott_strap_full_process_morningT.rds")
...
boot_strap_coef_reg_non_pond <- apply(boot_strap_full_process, 2, function(x) {
  ...
  rbind(
    ...
    df_diff_simple %>% mutate(weight = weight_fun) %>%
      ...
      summarise(
        normal_mean = mean(SYM_fatigueMatinale_echelle),
...
df_4meth <- boot_strap_coef_reg_non_pond %>%
  ...
  summarise(moy = mean(value), ci_sup = quantile(value, 0.975),
            ...
            .groups = "drop") %>%
  mutate(
    key = str_replace(key, "normal_mean", "Unadjusted mean comparison"),
...
df_smd <- df_modele_pds %>% select(all_of(c(Id, name_expo, Var_model_outcome))) # one_hot_fb(select_var$data_frame$df_analyse ,list_factor =  c(  "PPC_id_typeMasque"  ))
...
##################################################################################
# Print paper result
vec_cut <- read_rds("data/genere/obs_cut_morningT2.rds")
...
select_var$data_frame$df_analyse %>%
  summarise(
    epworth = mean(SYM_fatigueMatinale_echelle),
    sd = sd(SYM_fatigueMatinale_echelle),
    base_epworth = mean(Vis_init_SYM_fatigueMatinale_echelle),
    Sd_base = sd(Vis_init_SYM_fatigueMatinale_echelle),
    diff_epworth = median(Vis_init_SYM_fatigueMatinale_echelle - SYM_fatigueMatinale_echelle),
    SD_diff = sd(Vis_init_SYM_fatigueMatinale_echelle - SYM_fatigueMatinale_echelle)
  ) %>%
  arrondie_df(2)
...
select_var$data_frame$df_analyse %>%
  ...
  summarise(
    ...
    base_epworth = mean(Vis_init_SYM_fatigueMatinale_echelle),
    Sd_base = sd(Vis_init_SYM_fatigueMatinale_echelle),
    diff_epworth = mean(Vis_init_SYM_fatigueMatinale_echelle - SYM_fatigueMatinale_echelle),
    SD_diff = sd(Vis_init_SYM_fatigueMatinale_echelle - SYM_fatigueMatinale_echelle),
  ) %>%
  arrondie_df(2)


quantile(select_var$data_frame$df_analyse$Vis_init_SYM_fatigueMatinale_echelle, c(0.25, 0.75))
...
Init_ESS_mean_comp <- aov(Vis_init_SYM_fatigueMatinale_echelle ~ INS_obs_categ, donnee_boot)
summary(Init_ESS_mean_comp)


# Anova simple mean follow-up Epworth
Folow_ESS_mean_comp <- aov(SYM_fatigueMatinale_echelle ~ INS_obs_categ, donnee_boot)
summary(Folow_ESS_mean_comp)

# mean initial obs for all patients
obs_nume <- read_rds("data/genere/obs_morningT_quantit2.rds")
obs_nume %>%
...
#   "Vis_init_SYM_echelleDepression", "SYM_echelleDepression", "Vis_init_SYM_fatigueMatinale_echelle", "SYM_fatigueMatinale_echelle",
#   "Vis_init_SYM_cephaleesMatinales", "SYM_cephaleesMatinales", "Vis_init_SYM_fatigueMatinale_echelle", "SYM_fatigueMatinale_echelle",
