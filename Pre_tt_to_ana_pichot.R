...
Data_post_pre_tt <- read_rds("data/genere/data_prett.rds") %>% 
  select(-Mesure_unique_INS_visite_Max,-contains("fatigueMatinale_echelle")) %>% # remove instrumental var
...
df_EPW <- Data_post_pre_tt %>%
  ...
  group_by(id_patient) %>%
  mutate(naEPW = sum(is.na(SYM_echellePichot)|is.na(Vis_init_SYM_echellePichot))) %>% # utile car permet de scale sur plus de rendez-vous
  filter(naEPW == 0) %>% 
  select(-naEPW) %>% 
  ungroup()  
...
saveRDS(vec_cut, file = paste0("data/genere/obs_cut_pichot",visite_seuil,".rds"))

saveRDS(data.frame(id_patient = df_obs_categ$id_patient,PPC_observanceMoy_finale = df_obs_categ$PPC_observanceMoy_finale),
        file = paste0("data/genere/obs_pichot_quantit",visite_seuil,".rds"))
...
# Case where visit init and follow up are higly correlated 
...
df_post_imput <- impute_si_changement(df_pre_impute,"data/genere/data_impute_Pichot.rds",reimputation = FALSE)
...
if (df_post_imput %>% summarise_all(~sum(is.na(.))) %>% rowSums(.) != 0 ) {
  stop('Row avec Na')}
# no erectil disfunction beyond women
...
saveRDS(df_pre_impute, file = paste0("data/genere/data_pre_impute_Pichot_nb_vis_",visite_seuil,".rds"))
saveRDS(df_post_imput, file = paste0("data/genere/data_prett_Pichot_nb_vis_",visite_seuil,".rds"))
