...
write_rds(select_var,"data/genere/model_outcome.rds")
...
# Bootstrap comparison of treatment effect estimation
...
boot_strap_coef_reg_non_pond <- apply(boot_strap_full_process, 2, function(x) {
  ...
  df_diff_simple <- id_boot_strap %>% right_join(donnee_boot, by = c("id_patient")) # %>% select(-id_patient)
  rbind(
    ...
    df_diff_simple %>%
      ...
      summarise(
        normal_mean = mean(SYM_echelleEpworth),
...
df_4meth <- boot_strap_coef_reg_non_pond %>%
  ...
  mutate(key = factor(key, levels = c("Unadjusted mean comparison", "IPTW", "Multivariable linear regression", "IPWRA")))
...
select_var$data_frame$df_analyse %>%
  summarise(
    epworth = mean(SYM_echelleEpworth),
...
table_resume_rapport_latex_ss_escape_order %>%
  ...
  footnote(
    ...
    symbol = c(
      ...
      "1,2,3,4 numbers in subscript refers to columns statistically different at the 5% threshold. e.g. 1 means that there is a statistically significant difference between the adherence group \\textcolor{red}{of that column} and the 0-4 adherence group (1) for the variable in question."
...
df_box_plot_fig2 <- select_var$data_frame$df_analyse %>%
  ...
  rename("Epworth Sleepiness Scale score at diagnosis" = "Diagnosis epworth sleepiness scale",
         "Epworth Sleepiness Scale score at follow-up visit" = "Epworth sleepiness scale") %>%
  pivot_longer(-`Adherence groups`) %>%
  mutate(
    `Adherence groups` = as.factor(as.character(`Adherence groups`)),
    name = factor(name, levels = c(
      "Epworth Sleepiness Scale score at diagnosis",
      "Epworth Sleepiness Scale score at follow-up visit"
    ))
  )
...
ggplot(
  ...
  labs(
    y = "Epworth Sleepiness Scale score",
    x = paste0(""),
    fill = "CPAP adherence"
  )
...
df_smd_plot <- df_max %>%
  filter(rowname %in% c(Var_model_outcome))
...
df_plot_max <- table_rename$complete_table %>%
  ...
  mutate(Label = str_replace(
    ...
    Label = str_replace(
      Label, "Residual apnea  hypopnea index under CPAP",
      "Residual Apnea-Hypopnea Index under CPAP"
    ),
    Label = str_replace(
      Label, "Diagnosis epworth sleepiness scale",
      "Diagnosis Epworth Sleepiness Scale"
    ),
    Label = str_replace(
      Label, "Diagnosis pichot's fatigue scale",
      "Diagnosis Pichot's fatigue scale"
    )
...
df_figure_5 <- df_4meth %>%
  ...
  mutate(key = recode_factor(key, "Unadjusted mean comparison" = "Unadjusted",
                                    "IPWRA" = "IPW-RA"))
...
ggplot(
  ...
  facet_wrap(~key) +
  labs(x = "Adherence groups", y = "Difference in means Epworth Sleepiness Scale score with the reference group") +
  theme(
...
write("\\bibliographystyle{annalsofats}
...
# temp graph PS
model_res <- do.call(calcule_pds_stage, c(
  list(donne = donnee_boot ),
  params 
))$res_intermediaire$regression_modele_pds$regression_temps_ind




PS_long <- model_res$fitted.values %>%
  as.data.frame() %>% 
  pivot_longer(everything()) %>%
  mutate(name = as.factor(name))


levels(PS_long$name) <- groupe_obs_table_latex


PS_long %>% ggplot() + geom_density(aes(x = value,colour = name ))  + 
  labs(color='Observance à la CPAP',
       x = "Score de propension") 
ggsave("PS_density.jpeg",
       width = 12,
       height = 12,
       units = c("in")
)
