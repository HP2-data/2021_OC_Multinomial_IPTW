...
df_4meth <- boot_strap_coef_reg_non_pond %>% pivot_longer(-key) %>% group_by(key,name) %>% summarise(moy = mean(value),ci_sup = quantile(value,0.975),ci_inf = quantile(value,0.025) ,.groups = "drop") %>%
  mutate(key = str_replace(key,"normal_mean", "Unweighted mean comparaison"),
         key = str_replace(key,"reg_non_pond", "Unweighted linear regression"),
         key = str_replace(key,"reg_pond", "IPWRA")) %>%
  mutate(name = str_remove(name,"INS_obs_categ")) %>% 
  mutate(key = factor(key,levels = c("Unweighted mean comparaison","IPTW","Unweighted linear regression","IPWRA"))) 
...
Variables_core_paper <- c("INS_obs_categ","SYM_echelleEpworth","Vis_init_SYM_echelleEpworth","Mesure_unique_CHA_id_sexe","Vis_init_CHA_age","Vis_init_CHA_BMI","Vis_init_INS_IAH","PPC_IRAH","Vis_init_FDR_fumeur",
...
diag_var_table_latex <- table_resume_rapport_rename$complete_table %>% mutate(diagnostic = !is.na(preffix_all) ) %>% select(Label,diagnostic)
